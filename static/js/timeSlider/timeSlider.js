...
(function () {
  ...
  var defalutColor = ["1890FF", 'a5df12', 'eaaae4', '04d4d4', 'd32311'];
  ...
  function TimeSlider(options) {
    ...
    if (options.allInit) {
      g_instanceCtxArray = new Array();
      TimeSlider.prototype.sliderTotalNums = 0;
    }
  ...
  TimeSlider.prototype = {
    ...
    init: function () {
      ...
      // console.info("options.allInit2", this.options.allInit);
    ...
    createLayout: function () {
      ...
      // self.sliderLeftOffset = document.documentElement.scrollLeft;
...
创建时间段函数
    */
    ...
    startTimeBarDown: function (e, thisBar, timeBlockDom, startTimeShowDom) {
      ...
      document.onmouseup = document.onmouseleave = function () {
        ...
        // 重新获取startTimeCoordinate，解决双击时间快拖动为NAN值
        if (startTimeCoordinate === undefined) {
          startTimeCoordinate = parseFloat((getMousePos(e).x - self.sliderLeftOffset + mouseRelativeOffsetX).toFixed(4));
        }
    ...
    stopTimeBarDown: function (e, thisBar, timeBlockDom, stopTimeShowDom) {
      // console.info("结束参数", e, thisBar, timeBlockDom, stopTimeShowDom, oldStartTimeCoordinate)
      g_startClickTimeStamp = new Date().getTime();
      var self = this;
      ...
      var mouseRelativeOffsetX = parseFloat((getMousePos(e).x - self.sliderLeftOffset - (self.stopTimeCoordinateArray[self.whichOne] || 0)).toFixed(4)); //鼠标的相对位移
      //寻找边界值
      ...
      document.onmouseleave = document.onmouseup = function () {
        ...
        // 重新获取stopTimeCoordinate，解决双击时间快拖动为NAN值
        if (stopTimeCoordinate === undefined) {
          stopTimeCoordinate = parseFloat((getMousePos(e).x - self.sliderLeftOffset - mouseRelativeOffsetX).toFixed(4));
        }
  ...
  function createPopUpBox() {
                 ...
                 <div class="modal-dialog">\
                     <div class="modal-content">\
                        ...
                        <div class="modal-body">\
                          ...
                          <div class="event-div" style="display: none;">\
    ...
    setBtnDom.onclick = function () {
      ...
      if (stopMinInputDom.value >= 60) {
        stopMinInputDom.value = 59;
      }
      ...
      if (SPH < STH || (SPH == 24 && SPM > 0)) {
        ...
        return;
      } else if ((SPH * 60 + SPM) - (STH * 60 + STM) < self.minBlockTime) {
        //alert(["At least 30 minutes apart", "间隔至少" + self.minBlockTime + "分钟"][lan]);
        Message({
          message: "间隔至少" + self.minBlockTime + "分钟",
          type: "error",
        });
        return;
      }
  ...
  function creatEditDiv(context) {
    // <i class="el-icon-copy-document editImg" id="editImg"></i>\
 ...
 */
  function copyTimeSlider(flag, context) {
    ...
    // if (g_instanceCtxArray.length > 7) {
    //   g_instanceCtxArray.splice(0, 7);
    // }

    for (var j = 1; j <= 7; j++) {
      const dom = document.getElementById('editCBox' + self.timeSliderNums + "_" + j);
      if (dom.checked == true) {

        const currentCtx = g_instanceCtxArray[(j - 1)];

        if (flag) {
          removeAllTimeBlocks(currentCtx);
        }
        for (var i = 0; i < self.timeBlockNums; i++) {
          if (currentCtx) {
            currentCtx.createTimeBlock({
              startTime: self.startTimeArray[i],
              stopTime: self.stopTimeArray[i],
              info: self.infosArray[i]
            })
            currentCtx.infosArray[i] = self.infosArray[i];
          }
        }
      }
    }
