<template>
  <div class="router_container2 m20">
    ...
    <PDialog ref="inviteRef" @submit="comfirmAudit">
  ...
  methods: {
    ...
    async dialogShow(data) {
      ...
      await affairAudit({ affairsId: data.id }).then(res => {
        if (res.code === 10000) {
          ...
          this.checkParams.targetDeptName = res.data.affairsAudit.targetDeptName
        }
      })
    ...
    radioChange() {
      if (this.checkParams.auditResult === 0) {
        ...
        this.checkParams.targetDeptId = this.oneByDept.affairsAudit.operateDeptId
        this.checkParams.targetDeptName = this.oneByDept.affairsAudit.operateDeptName
      } else {
        this.checkParams.targetDeptId = this.oneByDept.affairsAudit.targetDeptId
        this.checkParams.targetDeptName = this.oneByDept.affairsAudit.targetDeptName
      }
    },
    comfirmAudit(data) {
      this.checkParams.targetDeptName = this.deptList.filter(i => { return i.value === this.checkParams.targetDeptId })[0].label
      console.log('this.checkParams', this.checkParams)
      // 执行 问政转移审核方法
      // fn...
      transferCheckAo(this.checkParams).then(res => {
        res.code === 10000 && (this.$message.success('审核成功'), this.checkShow = false)
      })
    },
    goDetail(data) {
