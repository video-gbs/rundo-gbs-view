<template>
  <div class="video-box">
    <video muted autoplay :id="idx" width="100%" style="background-color: #000;"></video>
  </div>
</template>

<script>
import Jessibuca from '../../../../../static/js/jessibuca/jessibuca'
export default {
    name: 'jessibuca',
    components: {
    },
      ...
      let paramUrl = decodeURIComponent(this.$route.params.url)
       this.$nextTick(() =>{
        ...
        //  this.play(this.videoUrl)
          ...
          this.videoUrl = this.videoUrl.replace(/.flv/g, ".mp4");
          console.info("this.videoUrl", this.videoUrl)
          this.player = new wsPlayer(this.idx, this.videoUrl);
          this.player.open();
              ...
              this.jessibuca.on("load", () => {
                ...
                try {
                  ...
                  if (!canvas.style.transform) {
                    canvas.style.transform = `scale(${dom.clientWidth / 300})`;
                    canvas.style.left = (dom.clientWidth - 300) / 2 + "px";
                    canvas.style.top = (dom.clientHeight - 150) / 2 + "px";
                  }
                } catch (error) {
                  console.info("处理视频canvas出错");
                }
              });
            }
        },
